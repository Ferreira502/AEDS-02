...
public class Heapsort{
    ...
    private static void Heapsort(Games[] array) {
        int n = array.length;

        for (int i = n / 2 - 1; i >= 0; i--) {
            aplicaMaxHeap(array, n, i);
        }

        for (int i = n - 1; i > 0; i--) {
            swap(array, 0, i);

            aplicaMaxHeap(array, i, 0);
        }
    }

    private static void aplicaMaxHeap(Games[] array, int n, int i) {
        int maior = i;
        int esquerda = 2 * i + 1; 
        int direita = 2 * i + 2; 

        if (esquerda < n && compareGames(array[esquerda], array[maior]) > 0) {
            maior = esquerda;
        }

        if (direita < n && compareGames(array[direita], array[maior]) > 0) {
            maior = direita;
        }
        ...
        if (maior != i) {
            ...
            aplicaMaxHeap(array, n, maior);
        }
    }

    private static void swap(Games[] array, int i, int j) {
        Games temp = array[i];
        array[i] = array[j];
        array[j] = temp;
        movimentacoes += 3;
